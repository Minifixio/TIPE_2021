...
def cercle_coeff(X,Y,n,canvas,R,debug=False,visu_state=False):
    global cercles
    global t
    t=t*2
    xmin = min(X)
...
def find_circle_coeff(data, canvas, R):
    ...
    if data != []:
        ...
        for i in range(-1,len(data)-1):
            p1=data[i]
            p2=data[i+1]
            canvas.create_line(p1[0],p1[1],p2[0],p2[1], fill="green", width=15*R[i])

        for p in data:
            X.append(p[0])
            Y.append(p[1])
        c=cercle_coeff(X, Y, 100, canvas, R)
        centre=c[0]
        xc=centre[0]
        yc=centre[1]
        r=c[1]
