...
canvas=None
debug_logs=True # Afficher les infos de debug
...
y_max=500 # Les bordures du cadre
x_max=500 # Les bordures du cadre
...
# Fonction principale
def fortunes(points):
    global Queue, Racine, Voronoi, y_pos
...
def dessin_point(point, taille, fill = "red"):
    x = point.x
    y = point.y
...
def dessin_ori_arete(arete):
    dessin_point(arete.origine, 2, fill="orange")

def dessin_poly(poly):
    for i in range(len(poly)-1):
        p1=poly[i]
        p2=poly[i+1]
        canvas.create_line(p1.x, p1.y, p2.x, p2.y, fill = "black", width=1)
    canvas.create_line(poly[-1].x, poly[-1].y, poly[0].x, poly[0].y, fill = "black", width=1)
    ...
    for arete in Voronoi:
        if arete:
            ...
            if debug_logs:
                debug(arete)
            if debug_grap:
                dessin_ori_arete(arete)
                
    # Ce sont les points verts ceux de Voronoi
    for sommet in Sommets:
        dessin_point(sommet, 8, fill="green")
    
    root.bind('<Button 1>', coord_souris)
    root.bind('<Motion>', mouvement)
    root.mainloop()

p_test=[ (118, 121), (294, 57), (359, 240), (327, 376), (165, 408), (200, 279)]
